...
public class HomeController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (path.endsWith("/Home")){
            ...
            if (path.endsWith("/Home/Trangchu")) {
                request.getRequestDispatcher("/trangchu.jsp").forward(request, response);
            } else {
                if (path.endsWith("/Home/Shop")) {
                    request.getRequestDispatcher("/shop-page.jsp").forward(request, response);
                } else {
                    if (path.startsWith("/Taikhoan/ChinhSua")) {
                        String[] s = path.split("/");
                        String masv = s[s.length - 1];
                        KhachHangDAO dao = new KhachHangDAO();
                        KhachHang kh = dao.GetKhachHangid(masv);
                        if (kh == null) {
                        ...
                        if(path.endsWith("/Dashboard")||path.endsWith("/Home/Dashboard")){
